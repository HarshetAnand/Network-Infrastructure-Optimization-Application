...
public class CS400Graph<NodeType, EdgeType extends Number> implements GraphADT<NodeType, EdgeType> {
	...
	protected Path dijkstrasShortestPath(NodeType start, NodeType end) {
		if ((!containsVertex(start)) || (!containsVertex(end))) {
			throw new NoSuchElementException("Error! No vertex containing start or end can be found!");
		} // Checks if there is a vertex contained at the start or end

		Hashtable<Vertex, Path> shortPath = new Hashtable<>();
		LinkedList<Vertex> nodesVisited = new LinkedList<>();
		Path newPath;

		PriorityQueue<Path> priorityQueue = new PriorityQueue<>();
		priorityQueue.add(new Path(vertices.get(start)));

		while (!priorityQueue.isEmpty()) { // Checks if queue is empty
			newPath = priorityQueue.poll();

			if (!(shortPath.get(vertices.get(newPath.end.data)) != null)) { // Checks if path is null
				shortPath.put(vertices.get(newPath.end.data), newPath);
				nodesVisited.add(vertices.get(newPath.end.data));

				for (Edge successor : vertices.get(newPath.end.data).edgesLeaving) {
					if (!nodesVisited.contains(successor.target)) {
						priorityQueue.add(new Path(newPath, successor)); // Adds new path
					}
				}
			}
		}
		if (!(shortPath.get(vertices.get(end)) != null)) {
			throw new NoSuchElementException("Error! No path from start to end can be found!");
		} // Checks if there is a path found from start to end

		return shortPath.get(vertices.get(end));
	}
